...
- **워커 하이라이트 경로 활성화됨**: `build.ts`가 `packages/diffs/src/worker/worker.ts`를 `dist/viewer/worker.js`로 번들하고, `main.ts`가 `getOrCreateWorkerPoolSingleton`(poolSize 2)으로 만든 풀을 CodeView에 주입한다. 워커 초기 렌더 옵션은 메인스레드 기대 옵션(`DiffHunksRenderer.getRenderOptions`의 5필드)과 일치해야 결과가 매치된다 — 불일치 시 색이 영영 안 입혀진다. 회귀망: `worker-highlight.e2e.ts`(첫 진입 무스파이크 + plain→색 전이 + 폴백).
