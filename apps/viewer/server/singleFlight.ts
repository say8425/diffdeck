 ...
 * 정상인데 프라미스만 안 끝난다. 출력이 클 수 있는 git 호출은 이제 전부
 * `Bun.spawn`(`gitOutput.ts`)을 타지만 이 타임아웃은 남겨 둔다 — 원인을 가리지
 * 않는 안전망이라(예: `baseFlight` 안의 `gh pr view`는 네트워크를 기다린다),
 * 그리고 남은 작은 `$`가 원리적으로 안전하다는 증명은 없다(60KB는 멀쩡했다는
 * 관측뿐이다). ShellPromise엔 `.timeout()`/`.kill()`이 없어 Promise.race가
