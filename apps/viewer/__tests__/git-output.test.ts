...
/**
 * `gitText`(와 그 아래 `gitBytes`)의 회귀망. 서버의 git 호출 중 출력이 64KB를
 * 넘을 수 있는 것은 전부 이 헬퍼를 탄다 — Bun 1.3.x의 `$`는 그런 호출에서
 * 자식이 끝났는데도 promise가 영영 settle하지 않을 수 있기 때문이다(업스트림은
 * 1.4.0에서 수정). 실제 호출처(예: `summary.ts`의 `diff --name-only`)는 걸리는
 * 빈도가 낮아 결정론적으로 재현되지 않으므로, 확실히 걸리는 모양 — 200KB
 * `git show` 12개를 8-way로 — 으로 헬퍼 자체를 찌른다.
 *
